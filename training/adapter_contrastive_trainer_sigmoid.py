"""
Stage 1 Training: Audio-Text Alignment (Sigmoid / SigLIP)

Trains the streaming adapter using SigLIP-style sigmoid contrastive learning to align
audio tokens with text embeddings from the frozen LLM.

Loss: L = L_align (sigmoid) + λ_stability · L_stability
"""
...
WANDB = WandbConfig(
    ...
    run_name="stage1-sigmoid-learnable-temperature",
)

CHECKPOINT_BASENAME = "adapter_sigmoid_stage1"
...
def _trainable_params(
    ...
    return [*adapter.parameters(), logit_scale, logit_bias]
...
def _make_stage1_checkpoint(
    *,
    ...
    return TrainingCheckpoint(
        ...
        contrastive_logit_bias=float(logit_bias.item()),
    )
...
def train():
    ...
    logit_bias = init_sigmoid_logit_bias(device=train_device)
    ...
    optimizer = torch.optim.AdamW(
        _trainable_params(adapter, logit_scale, logit_bias),
        lr=OPT.lr,
    ...
    if os.path.exists(SAVE_PATH):
        ...
        saved_logit_bias = ckpt.get("contrastive_logit_bias")
        if saved_logit_bias is not None:
            logit_bias.data.fill_(float(saved_logit_bias))
        ...
        if opt_state is not None:
            try:
                ...
                print(
                    "  [WARN] Optimizer state mismatch (e.g. new logit_scale/logit_bias params); "
    ...
    print("Starting Stage 1 training: Audio-Text Alignment (sigmoid)")
    ...
    print(f"  Logit bias (initial): {logit_bias.item():.4f}\n")
    ...
    for epoch in range(start_epoch, STAGE.epochs):
        ...
        for step, batch in enumerate(dataloader):
            ...
            params = _trainable_params(adapter, logit_scale, logit_bias)
                ...
                logger.log(
                    {
                        ...
                        "train/logit_bias": diag["logit_bias"],
