...
from training.utils.stage1_validation import validate_stage1_clap, wandb_val_log_dict
...
WANDB = WandbConfig(enabled=True, project="audio-streaming-adapter", run_name="stage1-CLAP-learnable-temperature")

CHECKPOINT_BASENAME = "adapter_clap_stage1"
...
#  check if the loss is working with a subset of the dataset   
    ...
    optimizer = torch.optim.AdamW(
        [*adapter.parameters(), logit_scale],
        lr=OPT.lr,
    ...
    if os.path.exists(SAVE_PATH):
            ...
            if saved_logit_scale is None:
                saved_logit_scale = ckpt.get("clap_logit_scale")
            ...
            # Checkpoints store 1-based epoch (see _maybe_save_step_checkpoint).
    ...
    for epoch in range(start_epoch, STAGE.epochs):
        ...
        for step, batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                label_embeds = text_embedder(label_ids).float().to(train_device)
            ...
            has_nan_grad = any(
                ...
                for param in [*adapter.parameters(), logit_scale]
            ...
            torch.nn.utils.clip_grad_norm_([*adapter.parameters(), logit_scale], OPT.grad_clip_norm)
            ...
            if (
                ...
                if os.path.isdir(VAL_ROOT):
                    ...
                    val_metrics = validate_stage1_clap(
                        adapter=adapter,
